...
#import "NSObject+TLReflection.h"
#import <objc/runtime.h>

@implementation NSObject (TLReflection)
...
{
    return [[self class] tt_propertyKeys];
}
+ (NSArray *)tt_propertyKeys {
...
{
    return [[self class] tt_propertiesInfo];
}
...
+ (NSArray *)tt_propertiesInfo
{
    ...
    for (int i = 0; i < propertyCount; i++)
    {
        [propertieArray addObject:({
            ...
            NSDictionary *dictionary = [self tt_dictionaryWithProperty:properties[i]];
...
+ (NSArray *)tt_propertiesWithCodeFormat
{
    ...
    NSArray *properties = [[self class] tt_propertiesInfo];
...
-(NSArray*)tt_methodListInfo{
    ...
    for (int i = 0; i < count ; i++)
    {
        ...
        for (int index=0; index<argumentsCount; index++) {
            ...
            [arguments addObject:[[self class] tt_decodeType:arg]];
        }
        
        NSString *returnTypeString =[[self class] tt_decodeType:returnType];
        NSString *encodeString = [[self class] tt_decodeType:encoding];
...
-(NSDictionary *)tt_protocolList{
    return [[self class]tt_protocolList];
}
+ (NSDictionary *)tt_protocolList
{
...
+ (NSArray *)tt_instanceVariable
{
    ...
    NSMutableArray *result = [NSMutableArray array];
    for (int i = 0; i < outCount; i++) {
        NSString *type = [[self class] tt_decodeType:ivar_getTypeEncoding(ivars[i])];
...
+ (NSDictionary *)tt_dictionaryWithProperty:(objc_property_t)property
{
...
+ (NSString *)tt_decodeType:(const char *)cString
{
    ...
    } else
    {
        if ([[result substringToIndex:1] isEqualToString:@"^"]) {
            result = [NSString stringWithFormat:@"%@ *",
                      [NSString tt_decodeType:[[result substringFromIndex:1] cStringUsingEncoding:NSUTF8StringEncoding]]];
        }
    }
    return result;
}
