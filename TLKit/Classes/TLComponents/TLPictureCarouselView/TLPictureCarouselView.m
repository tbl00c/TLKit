...
- (instancetype)initWithSelectAction:(void (^)(TLPictureCarouselView *pictureCarouselView, id<TLPictureCarouselProtocol> model))selectAction
{
    if (self = [self initWithFrame:CGRectZero]) {
        self.didSelectItem = selectAction;
    }
    return self;
}
...
- (void)setImageData:(NSArray *)imageData
{
    ...
    if (imageData.count > 1) {
        NSMutableArray *tempData = imageData.mutableCopy;
        [tempData insertObject:imageData.lastObject atIndex:0];
        [tempData addObject:imageData.firstObject];
        self.data = tempData;
    }
    else {
        self.data = imageData;
    ...
    [self.collectionView reloadData];
    [self.collectionView setPageX:self.data.count > 1 ? 1 : 0 animated:NO];
...
{
    if (scrollView.contentOffset.x <= 0) { // 向左滑动
        [scrollView setContentOffset:CGPointMake(scrollView.contentSize.width - scrollView.frame.size.width * 2, 0) animated:NO];
    }
    else if (scrollView.contentOffset.x >= scrollView.contentSize.width - scrollView.frame.size.width) { // 向右滑动
