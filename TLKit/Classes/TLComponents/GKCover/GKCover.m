...
{
    for (UIView *view in _cover.subviews.copy) {
        [view removeFromSuperview];
    }
    [_cover setBackgroundColor:[UIColor clearColor]];
    
    switch (_style) {
        case GKCoverStyleTranslucent: // 半透明
            [GKCover setupTranslucentCover:_cover];
            break;
        case GKCoverStyleTransparent: // 全透明
            [GKCover setupTransparentCover:_cover];
            break;
        case GKCoverStyleBlur:        // 高斯模糊
            [GKCover setupBlurCover:_cover];
            break;
...
+ (instancetype)translucentCoverWithTarget:(id)target action:(SEL)action
{
    ...
    cover.backgroundColor = [UIColor colorShadow];
    cover.alpha = kAlpha;
...
+ (void)showView
{
    ...
    if (_showStyle == GKCoverShowStyleTop) {
        ...
        CGPoint center = CGPointMake(_fromView.gk_width / 2, (_fromView.gk_height - 64) / 2);
        _contentView.gk_centerX = center.x;
...
+ (BOOL)darkMode
{
    return [TLTheme sharedInstance].themeStyle == TLThemeStyleDark;
}
